...
def generate_det_seed():
    ...
    n=int('FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141',16)
    while True:
        with open('BIP32/det_dict.txt') as dictionnary:
...
def derive_det_private_key(seed):
    return sha256(seed.encode('utf-8')).hexdigest().upper()
...
def derive_public_key(private_key):
    ...
    return private_key_point.public_key().public_numbers().encode_point()
...
def derive_key_pair(private_key):
    ...
    return [public_key.hex().upper(), address]

def generate_random_key_pair():
    private_key=generate_random_private_key()
    return [private_key]+derive_key_pair(private_key)
    #Tested against http://gobittest.appspot.com/Address

def generate_det_key_pair():
    private_key=derive_det_private_key(generate_det_seed())
    return [private_key]+derive_key_pair(private_key)
